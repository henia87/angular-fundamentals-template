...
export class AppComponent implements OnInit, OnDestroy {
  ...
  initCharacterEvents(): void {
    ...
    this.charactersResults$ = this.searchTermByCharacters
        .pipe(
        // YOUR CODE STARTS HERE
          filter((character) => character.length >= 3),
          debounceTime(500),
          switchMap((character) => this.mockDataService.getCharacters(character))
        // YOUR CODE ENDS HERE
        );
  }
  ...
  initLoadingState(): void {
    ...
    // YOUR CODE STARTS HERE
    let loadSub = combineLatest([this.mockDataService.getCharactersLoader(), this.mockDataService.getPlanetLoader()]).pipe(
      map(([charLoad, planetLoad]) => this.areAllValuesTrue([charLoad, planetLoad]))
    ).subscribe(isLoading => {
